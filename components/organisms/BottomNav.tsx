...
export function BottomNav() {
  ...
  const isBottomNavVisible = useGameStore(state => state.isBottomNavVisible);
    ...
    <AnimatePresence>
      {isBottomNavVisible && (
        <motion.nav 
          ...
          className="fixed bottom-4 sm:bottom-6 left-1/2 z-40 flex items-center justify-around bg-card/90 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-3 rounded-full neo-border neo-shadow-lg w-[95%] max-w-[400px]"
        >
          {items.map((item) => {
