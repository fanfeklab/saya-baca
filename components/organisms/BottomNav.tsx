...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center justify-around bg-card/90 dark:bg-slate-900/90 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-3 rounded-full neo-border neo-shadow-lg w-[95%] max-w-[400px] transition-colors">
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              item.prominent 
                ? "-top-4 sm:-top-6 bg-yellow-400 p-3 sm:p-4 rounded-full neo-border neo-shadow hover:scale-110 active:scale-95 text-neoblack" 
                : "p-2 hover:scale-110 active:scale-90 text-neoblack/60 dark:text-foreground/60",
              active === item.id && !item.prominent ? "text-neoblack dark:text-foreground scale-110 sm:scale-125" : ""
            )}
          >
            <Icon size={item.prominent ? 28 : 24} className={cn(active === item.id ? "stroke-[2.5px]" : "stroke-[2px]")} />
            {active === item.id && !item.prominent && (
               <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-neoblack dark:bg-foreground rounded-full" />
            )}
          </button>
        );
      })}
