...
export function MissionCard({ title, description, progress, total, onAction, className }: MissionCardProps) {
  ...
  return (
    <Card className={cn("bg-card border-2 border-black shadow-neo-lg overflow-visible", className)}>
      ...
      <CardContent className="space-y-6 px-6 pb-6">
        <div className="space-y-3">
          <div className="flex justify-between items-end">
            ...
            <div className="bg-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-black border-2 border-black shadow-neo-sm">
              {progress}/{total} MODUL
            </div>
          </div>
          <Progress value={percentage} className="h-4 border-2 border-black shadow-neo-sm bg-background" indicatorClassName="bg-primary" />
        </div>
        <ConfettiBurst>
          <Button variant="default" className="w-full text-xl h-16 shadow-neo active:shadow-none transition-all uppercase tracking-widest font-black border-2 border-black text-black" onClick={onAction}>
            Lanjutkan Belajar
          </Button>
        </ConfettiBurst>
      </CardContent>
...
export function StoryCard({ title, description, emoji, onRead, onListen, className }: StoryCardProps) {
  return (
    <Card 
      onClick={onRead}
      className={cn("bg-background flex flex-row items-center cursor-pointer hover:-translate-y-1 hover:shadow-neo p-4 gap-5 transition-all w-full border-2 border-black shadow-neo-sm translate-y-0 translate-x-0 active:translate-y-1 active:translate-x-1 active:shadow-none", className)}
    >
      <div className="shrink-0">
        <IllustrationHolder variant="secondary" size="lg" emoji={emoji || "📚"} className="rounded-2xl border-2 border-black shadow-neo-sm" />
      </div>
        ...
        <div className="flex gap-2">
          {onListen && (
            <AudioButton 
              variant="secondary" 
              size="sm" 
              className="w-fit text-[10px] font-black uppercase h-8 px-4 border-2 border-black shadow-neo-sm active:shadow-none translate-y-0 active:translate-y-1"
              onClick={(e) => {
                e.stopPropagation();
                onListen();
              }}
